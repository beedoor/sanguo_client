...
import com.game.sanguo.base.domain.ScanResourceData;
  ...
  extends GameTask
{
  ...
  protected void doExport()
  {
    ...
    try
    {
      ...
      fos.write(endline);
      fos.write("<resources>");
      writeOne(fos, this.searchResult.getTreasureList(), "treasure", endline);
      writeOne(fos, this.searchResult.getMarketList(), "market", endline);
      writeOne(fos, this.searchResult.getGoldList(), "gold", endline);
      writeOne(fos, this.searchResult.getSoliderList(), "solider", endline);
      fos.write("</resources>");
    }
    catch (Exception e)
    {
  ...
  private void writeOne(FileWriter fos, List<CityInfo> targetList, String type, String endline)
    throws Exception
  {
  ...
  private void msgIdGetZoneInfo(int areaX, int areaY)
  {
    PostMethod postMethod = new PostMethod(
      String.format(
      "%s/hero/dwr/call/plaincall/Multiple.2.dwr;jsessionid=%s;mid=%s", new Object[] {
      this.userBean.getUrlPrx(), this.userBean.getSessionId(), 
    ...
    postMethod.addParameter(new NameValuePair("httpSessionId", this.userBean
      .getSessionId()));
